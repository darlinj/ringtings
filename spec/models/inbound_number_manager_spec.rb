...
describe InboundNumberManager do
  ...
  describe "getting an inbound number from the database" do
    ...
    def do_get_free_number
      InboundNumberManager.get_free_number @callplan
    end
    it "grabs the a free number" do
      do_get_free_number
      @callplan.inbound_number.phone_number.should == @expected_number
    end
    describe "if there are no free numbers available" do
      it "will raise an exception of type NoPhoneNumbersAvailableException"
    end
  end
end
